  ...
  methods: {
    ...
    getValueFromMap: function getValueFromMap(value, map) {
      var matchedKeys = Object.keys(map).filter(function (key) {
        var val = map[key];
        var excluded = val && (typeof val === 'undefined' ? 'undefined' : _typeof(val)) === 'object' ? val.excluded : false;
        return excluded ? value < key : value <= key;
      }).sort(function (a, b) {
        return a - b;
      });
      var matchedValue = map[matchedKeys[0]];
      return matchedValue && (typeof matchedValue === 'undefined' ? 'undefined' : _typeof(matchedValue)) === 'object' ? matchedValue.value : matchedValue || '';
    },
